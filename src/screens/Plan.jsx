...
import { todayISO, addDays, weekISO, weekDaysFrom, fmtTime, agendaForDay, planEditMode } from '../lib/helpers.js';
import { DayPlanner } from './DayPlanner.jsx';

// Calendar/agenda: a week strip to pick a day, and a timeline merging that
// day's events, classes and scheduled tasks. A second tab hosts the Day
// Planner for building tomorrow during the nightly planning window.
export function Plan({
  ...
  const plannerEnabled = settings?.dayPlannerEnabled !== false;
  const win = settings?.planningWindow || { start: '20:00', end: '24:00' };
  ...
  return (
    <div style={{ paddingTop: 8 }}>
      {plannerEnabled && setTab && (
        <div style={{ display: 'flex', gap: 6, padding: '0 20px 4px' }}>
          {[['agenda', 'Agenda'], ['tomorrow', 'Tomorrow']].map(([v, label]) => (
            <button
              key={v}
              onClick={() => setTab(v)}
              style={{
                flex: 1, padding: '9px 0', borderRadius: 11, cursor: 'pointer',
                border: `0.5px solid ${tab === v ? C.amber : C.border}`,
                background: tab === v ? 'rgba(92,198,207,0.12)' : C.s1,
                color: tab === v ? C.amber : C.t2, fontSize: 13, fontWeight: 500,
              }}
            >{label}</button>
          ))}
        </div>
